...
const FilterPanel: React.FC<FilterPanelProps> = ({ 
  ...
  const uniqueRoles = Array.from(new Set(members.map(m => m.role).filter(Boolean))).map(r => ({ id: r, label: r }));

  const badgeClass = (isActive: boolean, activeColor: string) => 
    `cursor-pointer px-3 py-1 rounded-full text-xs font-medium border transition-colors select-none ${
      isActive 
